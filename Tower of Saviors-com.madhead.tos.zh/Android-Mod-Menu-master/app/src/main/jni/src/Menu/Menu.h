...
Java_uk_lgl_modmenu_FloatingModMenuService_Title(JNIEnv *env, jobject thiz) {
    titleValid = true;
    return env->NewStringUTF(OBFUSCATE("Modded by Yuki.kaco"));
}
...
Java_uk_lgl_modmenu_FloatingModMenuService_Heading(JNIEnv *env, jobject thiz) {
    headingValid = true;
    return env->NewStringUTF(OBFUSCATE("預設包滿不提醒,隊伍空間"));
}
...
Java_uk_lgl_modmenu_FloatingModMenuService_getFeatureList(JNIEnv *env, jobject activityObject) {
    ...
    const char *features[] = {
            OBFUSCATE("Category_感穴ZT,NANA,YUE"),
            OBFUSCATE("Toggle_進場勝利"),
            OBFUSCATE("Toggle_全體攻擊"),
            OBFUSCATE("Toggle_移除怪技 (全部)"),
            OBFUSCATE("Toggle_移除怪技 (普通+雙生)"),
            OBFUSCATE("Toggle_全域加速"),
            OBFUSCATE("Toggle_無限技能"),
            OBFUSCATE("InputValue_Combo")
